...
// user defines
#define SSD_JPEG_FIFO_SIZE_BLK      (4 * 1024)
#define SSD_JPEG_FIFO_SIZE_MAX      (128)
#define SSD_JPEG_FIFO_SIZE          (SSD_JPEG_FIFO_SIZE_BLK * 16)
#define SSD_JPEG_FIFO_START_ADDR    (0x30000)
#define SSD_JPEG_SCR_ADDR           (0)
#define SSD_JPEG_SCR_SIZE           (SSD_JPEG_FIFO_SIZE * 2)
#define SSD_JPEG_SEEK_END           2
#define SSD_JPEG_SEEK_SET           0

// structions and enums
typedef struct _SSD_JPEG_DECODE
{
    void    *stream;
    UINT16  orignal_width;
    UINT16  orignal_height;
    UINT16  display_width;
    UINT16  display_height;
    UINT16  op_mode;
    UINT32  fifo_addr;
    UINT32  fifo_addr_end;
    UINT32  size;
    UINT32  bytes_read;
}SSD_JPEG_DECODE;
...
// function prototypes
void SSDJPEGInit(void);
void SSDJPEGDecodeSetup(IMG_FILE_SYSTEM_API *data_cb, SSD_JPEG_DECODE *decode);
SSD_JPEG_ERR SSDJPEGDecodeHeader(IMG_FILE_SYSTEM_API *data_cb, SSD_JPEG_DECODE *decode);
void SSDJPEGPutBuffer(SSD_JPEG_DECODE *decode, BYTE *buffer, UINT32 size);
SSD_JPEG_ERR SSDJPEGDecodeStart(SSD_JPEG_DECODE *decode);
SSD_JPEG_ERR SSDJPEGDecodeData(IMG_FILE_SYSTEM_API *data_cb, SSD_JPEG_DECODE *decode);
SSD_JPEG_ERR SSDJPEGDecodeWait(SSD_JPEG_DECODE *decode);
SSD_JPEG_ERR SSDJPEGDecodeStop(void);
void SSDJPEGPutByte(SSD_JPEG_DECODE *decode, BYTE data);
void SSDJPEGConvertYUVRGB565(UINT16 x, UINT16 y, UINT16 width, UINT16 height, UINT32 buffer_addr);
