...
// default time and date settings at start up
#define STARTHOUR               10
#define STARTMINUTE             10
#define STARTSECOND             1
#define STARTDAY                15
...
const FONT_FLASH            *ptrSmallAsianFont = &CHINESE_SMALL;
...
int main(void)
{
    ...
    #if (GRAPHICS_HARDWARE_PLATFORM == GFX_PICTAIL_V1) || (GRAPHICS_HARDWARE_PLATFORM == GFX_PICTAIL_V2)
	...
	#else
		#warning "SPI memory not initialized"    
    #endif
...
void StartScreen(void)
{
    ...
    SHORT   counter;
    XCHAR   *text = (XCHAR *)TouchScreenStr;    // static const XCHAR text[] = "Touch screen to continue";
    ...
    MoveTo
    (
        ((GetMaxX() - GetTextWidth(text, (void *)ptrLargeAsianFont)) >> 1),
        ((GetMaxY() - GetTextHeight((void *)ptrLargeAsianFont)) >> 1)
    );
...
//    // initialize date and time
    ...
    RTCCSetBinMin(STARTMINUTE);     // set the minute value
    RTCCSetBinSec(STARTSECOND);     // set the second value
    RTCCSetBinDay(STARTDAY);        // set the day value
    RTCCSetBinMonth(STARTMONTH);    // set the month value
    RTCCSetBinYear(STARTYEAR);      // set the year value
...
"BACK......\n" "Meter\n" "Dial\n" "Picture\n" "Custom control\n" "Signature\n" "Plotting\n" "ECG"; 
*/
XCHAR   *ScreenItems = (XCHAR *)ScrSelList1;
XCHAR   *ScreenItems2 = (XCHAR *)ScrSelList2;
...
XCHAR       DTPlusSym[] = {'+',0};
XCHAR       DTMinusSym[] = {'-',0};
XCHAR       *DTDoneText = (XCHAR *)ExitStr;
XCHAR       *DTDownArrow = (XCHAR *)DownArrowStr;
XCHAR       *DTUpArrow = (XCHAR *)UpArrowStr;
...
#define MINMETER2VALUE  1

// Creates meter screen
void CreateMeter(void)
{
	
    #define MTR_WIDTH    		128  
    #define MTR_HEIGHT    		131  
    #define MTR_ORIGIN_X   		(((GetMaxX()>> 1) - MTR_WIDTH) - 2) 
    #define MTR_ORIGIN_Y    	((40 + GetMaxY() - 180 + 1) / 2)

    #define MTR_BTN_WIDTH		(MTR_WIDTH >> 1)
    #define MTR_BTN_ORIGIN_X    (MTR_ORIGIN_X + (MTR_WIDTH >>2))
    ...
    pMtr = MtrCreate
        (
            ...
            MTR_ORIGIN_X,
            MTR_ORIGIN_Y,
            MTR_ORIGIN_X + MTR_WIDTH, 
            MTR_ORIGIN_Y + MTR_HEIGHT,     // set dimension
    ...
    pMtr = MtrCreate
        (
            ID_METER2,
            MTR_ORIGIN_X + MTR_WIDTH + 4, //MTR_ORIGIN_X + 128,
            MTR_ORIGIN_Y + 0,
            MTR_ORIGIN_X + (MTR_WIDTH<<1) + 4, //MTR_ORIGIN_X + 258,
            MTR_ORIGIN_Y + MTR_HEIGHT, // set dimension
    ...
    // set title and values fonts
    MtrSetTitleFont(pMtr, (void *) &CHINESE_SMALL);
    ...
    (
        ...
        MTR_BTN_ORIGIN_X + MTR_BTN_WIDTH,
        MTR_ORIGIN_Y + 180,     // right, bottom corner
        0,                      // draw rectangular button	
    ...
    (
        ID_BUTTON2,             // button ID
        MTR_BTN_ORIGIN_X + (MTR_BTN_WIDTH<<1),
        MTR_ORIGIN_Y + 140,     // left, top corner	
        MTR_BTN_ORIGIN_X + (MTR_BTN_WIDTH<<1) + MTR_BTN_WIDTH,
        MTR_ORIGIN_Y + 180,     // right, bottom corner
        0,                      // draw rectangular button	
        BTN_DRAW | BTN_TOGGLE,  // will be dislayed after creation
        NULL,                   // no bitmap	
    ...
    for(i = ID_BUTTON1; i < ID_BUTTON2 + 1; i++)
    {
        ...
        if(i == ID_BUTTON1)
        {
            ...
            if(GetState(pObj, BTN_PRESSED))                     // increment meter value when button
                MtrSetVal(pMtrObj, value + 2);                  // is pressed
            else
                MtrSetVal(pMtrObj, value - 2);                  // decrement meter value when button
...
void CreateDial(void)
{
    ...
    GOLFree();                          // free memory for the objects in the previous linked list and start new list
    #if !(defined(__dsPIC33FJ128GP804__) || defined(__PIC24HJ128GP504__) || defined(__PIC24FJ256DA210__))
    ...
    (
        ...
        DIAL_ORIGIN_Y + 0,
        DIAL_ORIGIN_X + 247,
        DIAL_ORIGIN_Y + 131,            // set dimension
    ...
    // set values fonts
    MtrSetValueFont(GOLFindObject(ID_METER1), (void *) &CHINESE_SMALL);
}
