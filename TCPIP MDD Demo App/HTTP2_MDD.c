...
static BOOL HTTPSendFile(void)
{
	...
	switch(smHTTPSendFile)
	{
		...
		case SM_PARSE_TILL_DYN_VAR:
			...
			else
			{
				...
				if(availbleTcpBuffSize >= 64)
				{
					...
					{
						len=FSfread(databuffer, 64, 1, curHTTP.file);
						TCPPutArray(sktHTTP, databuffer, 64);
						numBytes -=64;
						bytesReadCount+=64;
					}
					else
					{
						len=FSfread(databuffer, (dynVarRcrdOffset-bytesReadCount), 1, curHTTP.file);
		
						TCPPutArray(sktHTTP, databuffer, (dynVarRcrdOffset-bytesReadCount));
				...
				else if(availbleTcpBuffSize != 0)
				{
					if(bytesReadCount != dynVarRcrdOffset )
					{
						...
						{
							len=FSfread(databuffer, availbleTcpBuffSize, 1, curHTTP.file);
							TCPPutArray(sktHTTP, databuffer, availbleTcpBuffSize);
		...
		case SM_PARSE_DYN_VAR_STRING:
			...
			len=FSfread(databuffer, 1, 1, curHTTP.file);
			numBytes-=1;
			bytesReadCount+=1;
			if(databuffer[0]=='~')
			{
		...
		case SM_SERVE_TEXT_DATA:
			...
			if(availbleTcpBuffSize >= 64)
			{
				...
				{
					len=FSfread(databuffer, numBytes, 1, curHTTP.file);
					TCPPutArray(sktHTTP, databuffer, numBytes);
					numBytes=0;
...
void HTTPIncFile(ROM BYTE* cFile)
{
	...
	if(availbleTcpBuffSize >= 64)
	{
		if(numBytes <= 64)
		{
			len=FSfread(databuffer, numBytes, 1, fp);
			TCPPutArray(sktHTTP, databuffer, numBytes);
			numBytes=0;
		}	
		else
		{	
			len=FSfread(databuffer, 64, 1, fp);
			TCPPutArray(sktHTTP, databuffer, 64);
			numBytes -=64;
		}
	}
	...
	{	
		...
		{
			len=FSfread(databuffer, availbleTcpBuffSize, 1, fp);
			TCPPutArray(sktHTTP, databuffer, availbleTcpBuffSize);
			numBytes -=availbleTcpBuffSize;
		}
	}
