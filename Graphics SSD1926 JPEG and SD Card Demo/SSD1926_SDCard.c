...
inline volatile DWORD __attribute__((always_inline)) GetCommandResponse(BYTE idx)
{
    DWORD_VAL rsp;
...
DWORD RCA;
   ...
   if(!SDSendCommand(CMD_SEND_RCA, SSD_RESPONSE_48 | SSD_CMD_CRC_CHK | SSD_CMD_IDX_CHK, 0xFFFFFFFF))
    ...
    while(!(GetReg(0x1130)&0x01)); //check cmd complt int
...
BYTE SDSectorRead(DWORD sector_addr, BYTE* buffer)
{
    ...
    // Send command
	if(!SDSendCommand(CMD_RD_SINGLE, SSD_RESPONSE_48 | SSD_DATA_PRESENT | SSD_CMD_CRC_CHK | SSD_CMD_IDX_CHK, sector_addr*sectorSize))
        return FALSE; 
    
    // Wait for transfer complete   
    while(!(GetReg(0x1130)&0x20) /*&& !(GetReg(0x1125) & 0x08)*/);
...
SetReg(0x1130, 0x08); //clear previous interrupt
	...
	if(!SDSendCommand(CMD_RD_MULTIPLE, SSD_RESPONSE_48 | SSD_DATA_PRESENT | SSD_CMD_CRC_CHK | SSD_CMD_IDX_CHK, sector_addr*sectorSize))
        return FALSE; 
    ...
    if(!SDSendCommand(CMD_STOP_TRANSMISSION, SSD_CMD_TYPE_ABORT | SSD_RESPONSE_48_BUSY | SSD_CMD_CRC_CHK | SSD_CMD_IDX_CHK, 0xFFFFFFFF))
        return FALSE; 
    SetReg(0x1130, 0x01); //clear previous interrupt
...
UINT16 i;
    ...
    //Normal interrupt handling
    SetReg(0x1134, GetReg(0x1134)|0x13); //enable write interrupt
    SetReg(0x1130, 0x13); //clear all type of normal interrupt

    //Error interrupt handling
    SetReg(0x1136,GetReg(0x1136)); // clear previous interrupt
    SetReg(0x1132,0xFF); // clear previous interrupt
    SetReg(0x1133,0x01); // clear previous interrupt

	if(!SDSendCommand(CMD_WR_SINGLE, SSD_RESPONSE_48 | SSD_DATA_PRESENT | SSD_CMD_CRC_CHK | SSD_CMD_IDX_CHK, sector_addr*sectorSize))
        return FALSE; 
