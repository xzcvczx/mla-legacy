...
// Array to hold cumulative power values for sensors
extern INT64 gCumulativePower[3];
...
INT32 GetUptimeSec(void)
{
    if (StackStartTime < 1248245820)
        ...
        return SNTPGetUTCSeconds() - StackStartTime;
}
...
{
    #define POWER_ADD           TRUE
    #define POWER_SUBTRACT      FALSE

    static BYTE step = 0;
    static BOOL stepDirection = POWER_ADD;
    DWORD ledValues;
    DWORD powerConsumed;
    BYTE n;
    ...
    *timestamp = GetUtcTimeSec();
    ...
    // Switch based on which sensor the code is acessing
    // Sensor 0 (CM_TRIANGLE) will generate a triangle wave-shaped power curve
    // Sensor 1 (CM_POT) will generate a power curve proportional to the value
    //  read from the board's potentiometer
    // Sensor 3 (CM_LEDS) will generate a value equivalent to the power
    //  consumption of a 60W light bulb for every LED that's active (use the
    //  push buttons to toggle LEDs)
    switch (which)
    {
        case CM_TRIANGLE:
            gCumulativePower[CM_TRIANGLE] += (INT64)step;
            if (stepDirection == POWER_ADD)
            {
                step += 10;
                if (step == 60)
                {
                    stepDirection = POWER_SUBTRACT;
                }
            }
            else
            {
        ...
        case CM_POT:
            	...
            	// AD converter errata work around (ex: PIC18F87J10 A2)
            		...
            		ADCON2 = PRODL;
            	#endif
                powerConsumed = ADRESL;
            #endif
            // Divide the A/D result to give a value
            // between ~0-60 W*(10-min).
            // This will be represented as a value of 0-360 W*h
            // on the PowerMeter display
            powerConsumed /= 17;
            gCumulativePower[CM_POT] += (INT64) powerConsumed;
            break;
        case CM_LEDS:
            ledValues = LED_GET();
            ledValues &= 0x1E;
            ...
            // These represent the number of "60W light bulbs" on
            for(n=0; ledValues; n++)
                ledValues &= ledValues-1;
            // Multiply the number of "Light Bulbs" by the power
            // consumption of each light bulb (60W).  Then multiply by
            // the number of seconds in a sampling period and divide
            // by the number of seconds in an hour.  This will give
            // us the number of watt*hours consumed in the capture interval
            powerConsumed =  (DWORD)n * 60 * (DWORD)GetPreferenceInt32(CAPTURE_SEC_INT);
            powerConsumed /= 3600;
            // Update the cumulative power
            gCumulativePower[CM_LEDS] += (INT64) powerConsumed;
            break;
    }

    // Set the varaible pointed to by 'watts' to the new, updated cumulative power
    *watts = gCumulativePower[which];
}
...
UINT32 GetSQ(INT16 which)
{
    ...
    value = GetCounter (G_SEND_ATTEMPT0) + GetCounter (G_SEND_ATTEMPT1) + GetCounter (G_SEND_ATTEMPT2);
...
void GetSQDetails(INT16 which, UINT32 *count, UINT32 *skipped)
{
    ...
    if (count != NULL)
    {
        val = GetCounter (G_SEND_ATTEMPT0 + which);
...
INT8 SensorStillActive(INT16 which_sensor)
{
    switch (which_sensor)
    {
        // None of our sensors will ever be turned off or detatched
        // so just return TRUE
        case CM_TRIANGLE:
        case CM_POT:
        case CM_LEDS:
