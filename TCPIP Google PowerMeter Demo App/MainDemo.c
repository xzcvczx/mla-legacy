...
PMButtonState buttonState;
// Representation of our LEDS
BYTE ledBits;
...
#endif
{
    ...
    gCumulativePower[0] = 0;
    gCumulativePower[1] = 0;
    gCumulativePower[2] = 0;
    ledBits = 0x00;
    LED_PUT (ledBits);
...
//	}
    ...
    gPowerMeterPreferences.pKeyHashes[0][0] = 0x00;
    gPowerMeterPreferences.pKeyHashes[1][0] = 0x00;
    gPowerMeterPreferences.pKeyHashes[2][0] = 0x00;
	...
	#if defined(EEPROM_CS_TRIS)
	{
		...
		// contents.
		XEEReadArray(identifierOffset, &c, 1);
	    if(c == APPLICATION_BUILD)
        {
		    XEEReadArray(appConfigOffset, (BYTE*)&AppConfig, sizeof(AppConfig));
	...
	#elif defined(SPIFLASH_CS_TRIS)
	{
		...
		SPIFlashReadArray(identifierOffset, &c, 1);
		if(c == APPLICATION_BUILD)
        {
			SPIFlashReadArray(appConfigOffset, (BYTE*)&AppConfig, sizeof(AppConfig));
...
void SaveNVMemContents(void)
{
	...
	#if defined(EEPROM_CS_TRIS)
	    XEEBeginWrite(identifierOffset);
	    XEEWrite(APPLICATION_BUILD);
        XEEEndWrite();
    #else
	    SPIFlashBeginWrite(identifierOffset);
	    SPIFlashWrite(APPLICATION_BUILD);
    #endif
...
{
	#if defined(EEPROM_CS_TRIS)
	    XEEBeginWrite(appConfigOffset);
	    XEEWriteArray((BYTE*)&AppConfig, sizeof(AppConfig));
        XEEEndWrite();
    #else
	    SPIFlashBeginWrite(appConfigOffset);
...
{
	#if defined(EEPROM_CS_TRIS)
	    XEEBeginWrite(0x0001 + sizeof(AppConfig));
	    XEEWriteArray((BYTE*)&gPowerMeterPreferences, sizeof(gPowerMeterPreferences));
        XEEEndWrite();
    #else
	    SPIFlashBeginWrite(0x0001 + sizeof(AppConfig));
