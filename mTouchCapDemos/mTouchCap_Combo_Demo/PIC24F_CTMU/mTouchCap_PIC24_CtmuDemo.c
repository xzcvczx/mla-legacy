    ...
    while(1)
    {
        ...
        if(dataReadyCTMU == 1)                      // This flag is set by Timer 4 ISR //when all channels have been read										
        {
			...
			***************************************************************************/
			...
			/***************************************************************************
			--------------- Demo of API "mTouchCapAPI_SetUpChannel" -----------------------		
			This API is used to setup the channel based on the parameter passed.
			***************************************************************************/
			mTouchCapAPI_SetUpChannel (SLDR2CHNL1, DEFAULT_TRIP_VALUE, DECODE_METHOD_PRESS_ASSERT, FILTER_METHOD_1BY20);
		#endif
			...
			***************************************************************************/
			...
			assigned/initalized in "mTouchCAp_Config.h" file. Also enable "USE_ONE_CHANNEL_BOARD" option in configuration */
            #ifdef USE_ONE_CHANNEL_BOARD
            DemoChannel = SINGLE_KEY;
            AdcValueToAchieve = 0x2FF;              //Sample ADC count to achieve. 75% of max/ 0x3ff*0.75 = 0x2FF. This can be changed by user.
