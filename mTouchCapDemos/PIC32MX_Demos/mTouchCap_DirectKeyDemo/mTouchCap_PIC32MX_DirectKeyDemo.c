...
int main(void)
{
    ...
    while(1)
    {
        ...
        if(dataReadyCVD == 1)  //This flag is set by Timer ISR when all channels have been read
        {
            ...
            CurrentButtonStatus = mTouchCapApp_DemoDirectKeys();
            ...
            if ( CurrentButtonStatus > 0 ) // Something is happening on the buttons
            {
                ...
                for ( iButton = 0; iButton < 8 ; iButton++ )
                {
                    ...
                    SliderValue += WeightDeviation*(iButton+1);
                }
                SliderValue = (256/8)*SliderValue/WeightSum; // Stuff this value out the UART to be displayed on PC mTouch GUI
            }
...
#          if defined( UART_DUMP_RAW_COUNTS ) || defined( UART_DUMP_ALL_COUNTS )
            {
                sprintf(ButtonMeasString,
                        ...
                        iHF_Read+1, CurrentButtonStatus, CurrentButtonAsserts, SliderValue,
