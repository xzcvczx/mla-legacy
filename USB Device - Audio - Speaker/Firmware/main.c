...
********************************************************************
 ...
 Change History:
  ...
  2.6   11/11/2009   Initial release
********************************************************************/
...
UINT8 ReceivedDataEvenBuffer[NO_OF_SAMPLES_IN_A_USB_FRAME];// Holds the Even Audio Data received from Host
UINT8 ReceivedDataOddBuffer[NO_OF_SAMPLES_IN_A_USB_FRAME]; // Holds the Odd  Audio Data received from Host
...
UINT8 *nextSample;  // Points to the Sample to be played next.  
	...
	void YourLowPriorityISRCode()
	{
		...
		{	
			...
			if (sampleCounter < NO_OF_SAMPLES_IN_A_USB_FRAME)
			{
		        ...
		        // Load the next sample to the PWM Duty Cycle Register. 
				DUTY_CYCLE = *nextSample; 
				nextSample++;
				sampleCounter++;  
			}	
			else if (evenIsBeingPlayed)
			{
				...
				if (newOddData)//Check if Audio Samples are waiting to be played in Odd Buffer 
				{
					...
					DUTY_CYCLE = ReceivedDataOddBuffer[0]; // Load the first Sample.
			...
			{ 
				...
				{
					// We need to start the Playing the audio samples in the Odd buffer immediately. 
					DUTY_CYCLE = ReceivedDataEvenBuffer[0];// Load the first Sample.
    ...
    void __attribute__((interrupt, no_auto_psv)) _OC1Interrupt(void)
    {
 	...
 	void __ISR(_TIMER_2_VECTOR, ipl7) TimerHandler (void)
 	{
		...
		if (sampleCounter < NO_OF_SAMPLES_IN_A_USB_FRAME)
		{
    		//If the code gets to this point, we are in the middle of an audio playback. 
    		// Load the next sample to the PWM Duty Cycle Register. 
			DUTY_CYCLE  = (*nextSample) * MULTIPLY_FACTOR; 
			nextSample++;
			sampleCounter++;  
		}		 
		else if (evenIsBeingPlayed)
		{
			...
			{
				// We need to start the Playing the audio samples in the Odd buffer immediately. 
				DUTY_CYCLE = ReceivedDataOddBuffer[0]*4; // Load the first Sample. 
				nextSample = &ReceivedDataOddBuffer[1]; // Set the adress for the second sample. 
				oddIsBeingPlayed = TRUE; 
				sampleCounter = 1;
			}  
		...
		{ 
			...
			{
				// We need to start the Playing the audio samples in the Even buffer immediately.
				DUTY_CYCLE = ReceivedDataEvenBuffer[0]*4;// Load the first Sample. 
				nextSample = &ReceivedDataEvenBuffer[1]; // Set the adress for the second sample.
				evenIsBeingPlayed = TRUE;  
				sampleCounter = 1;
			}
 ...
 *******************************************************************/
 ...
 void void UsbAudioInputTerminalControlRequestsHandler(void)
 {
...
{   
 	...
 	if (receivedDataEvenNeedsServicingNext == TRUE) //Check which buffer (even/odd) the next set of data is going to arrive in
	{
    	if(!USBHandleBusy(USBRxEvenHandle))//Check if the endpoint has received any data from the host.
    	{
        	...
        	USBRxEvenHandle = USBRxOnePacket(AS_EP,(BYTE*)&ReceivedDataEvenBuffer,NO_OF_SAMPLES_IN_A_USB_FRAME);
	...
	else //else Odd Endpoint needs servicing next
	{
		if(!USBHandleBusy(USBRxOddHandle))//Check if the endpoint has received any data from the host.
		{
			...
			//Get ready for next packet (this will overwrite the old data)
        	USBRxOddHandle = USBRxOnePacket(AS_EP,(BYTE*)&ReceivedDataOddBuffer,NO_OF_SAMPLES_IN_A_USB_FRAME);
...
void USBCBInitEP(void)
{
    ...
    USBRxEvenHandle = USBRxOnePacket(AS_EP,(BYTE*)&ReceivedDataEvenBuffer,NO_OF_SAMPLES_IN_A_USB_FRAME); //First Audio Packet sent will arrive in the even buffer.
	USBRxOddHandle = USBRxOnePacket(AS_EP,(BYTE*)&ReceivedDataOddBuffer,NO_OF_SAMPLES_IN_A_USB_FRAME); //Second Audio Packet sent will arrive in the even buffer.
