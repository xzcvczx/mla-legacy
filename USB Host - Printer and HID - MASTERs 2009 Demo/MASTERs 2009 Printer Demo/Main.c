/******************************************************************************
    MASTERs 2009 USB Embedded Host POS Printer Demo
...
#define MASTERS_DEMO_VERSION            0x0294
#define MASTERS_DEMO_VERSION_STRING     "v2.94"
...
#define NUM_PRIZE_DAYS                  (PRIZE_DAY_LAST - PRIZE_DAY_FIRST + 1)        // 3, cannot cross a month
#define NUM_PRIZES                      6
#define PASSWORD_LENGTH_MAX             30
#define PRIZE_ACCESS_PASSWORD           "0129**0815"
#define PRIZE_BARCODE_LENGTH            7               // Includes 0 / check digit
#define PRIZE_DAY_FIRST                 29
#define PRIZE_DAY_LAST                  31
#define PRIZE_MONTH                     7               // July
#define PRIZE_YEAR                      9               // 2009
...
#define DEFAULT_YEARS               0x0009
#define DEFAULT_MONTH_DAY           0x0727
...
BOOL DataCollectionHandler(void);
...
extern BITMAP_FLASH guitarHero;
extern BITMAP_FLASH nikeiPod;
...
extern BITMAP_FLASH samplePack;
extern BITMAP_FLASH tshirt;
extern BITMAP_FLASH travelMug;
...
PRIZE                   prizes[NUM_PRIZES]  = {  // In descending order of "value".  Sum of rates must be <= 16 {available, given, rate}
                                                { "Guitar Hero",        {0, (FLASH_BYTE*)0}, "GUITAR", {{  1, 0, 1}, {  1, 0, 1}, {  1, 0, 1}} },
                                                { "PIC24F Starter Kit", {0, (FLASH_BYTE*)0}, "P24SK ", {{  1, 0, 1}, {  1, 0, 1}, {  1, 0, 1}} },
                                                { "Nike + iPod",        {0, (FLASH_BYTE*)0}, "NIPOD",  {{  3, 0, 2}, {  3, 0, 2}, {  3, 0, 2}} },
                                                { "XLP Sample Pack",    {0, (FLASH_BYTE*)0}, "XLPPIC", {{  3, 0, 2}, {  3, 0, 2}, {  3, 0, 2}} },
                                                { "Travel Mug",         {0, (FLASH_BYTE*)0}, "REDMUG", {{  2, 0, 1}, {  2, 0, 1}, {  2, 0, 1}} },
                                                { "T-shirt",            {0, (FLASH_BYTE*)0}, "TSHIRT", {{ 24, 0, 4}, { 24, 0, 4}, { 24, 0, 4}} } };
BYTE                    prizeWon;
HID_REPORT_BUFFER       reportScanner;
XCHAR                   scanBarcode[]       = "Scan a barcode...";
USB_HID_DEVICE_ID       scannerID;
...
BOOL USB_ApplicationEventHandler( BYTE address, USB_EVENT event, void *data, DWORD size )
{
    switch( event )
    {
        ...
        case EVENT_HID_ATTACH:
            scannerID = *(USB_HID_DEVICE_ID *)data;
            return TRUE;

        case EVENT_HID_DETACH:
            scannerID.deviceAddress = 0;
            return TRUE;
...
void PrintExample( void )
{
    ...
    // Text
    USBHostPrinterCommandWithReadyWait( &returnCode, printerInfo.deviceAddress, USB_PRINTER_POS_TEXT_LINE, USB_DATA_POINTER_RAM("My Shopping List"), 3, USB_PRINTER_TRANSFER_COPY_DATA );
    ...
    // Set up all barcodes
//                            params.sBarCode.x;              // Horizontal location of the lower left corner of the bar code.
//                            params.sBarCode.y;              // Vertical location of the lower left corner of the bar code.  Readable text will print below this point.
    params.sBarCode.height          = 75;         // Bar code height in dots.
//                            params.sBarCode.width;          // Bar code width.  Valid values are dependent on the particular POS printer.
    ...
    //params.sBarCode.textSize;       // Size of the readable text.  Valid values are dependent on the particular POS printer.
    params.sBarCode.flags.bits.bPrintCheckDigit = 0;
    ...
    strcpy( bufferItem, "DM240001" );
    params.sBarCode.data            = (BYTE *)bufferItem;          // Pointer to the barcode data.
    params.sBarCode.dataLength      = strlen(bufferItem);      // Number of bytes of barcode data.
    ...
    // Item
    USBHostPrinterCommandWithReadyWait( &returnCode, printerInfo.deviceAddress, USB_PRINTER_POS_FEED, USB_NULL, 2, USB_PRINTER_TRANSFER_COPY_DATA );
    USBHostPrinterCommandWithReadyWait( &returnCode, printerInfo.deviceAddress, USB_PRINTER_POS_TEXT_LINE, USB_DATA_POINTER_RAM("AC164127 Graphics PICtail Plus Daughter Board   $135.00"), 1, USB_PRINTER_TRANSFER_COPY_DATA );
    strcpy( bufferItem, "AC164127" );
    params.sBarCode.data            = (BYTE *)bufferItem;          // Pointer to the barcode data.
    params.sBarCode.dataLength      = strlen(bufferItem);      // Number of bytes of barcode data.
    USBHostPrinterCommandWithReadyWait( &returnCode, printerInfo.deviceAddress, USB_PRINTER_POS_BARCODE, USB_DATA_POINTER_RAM(&params.sBarCode), sizeof(params.sBarCode), 0 );
    ...
    //Feed
    USBHostPrinterCommandWithReadyWait( &returnCode, printerInfo.deviceAddress, USB_PRINTER_POS_FEED, USB_NULL, 3, USB_PRINTER_TRANSFER_COPY_DATA );
    // Double height
    ...
    // Text
    USBHostPrinterCommandWithReadyWait( &returnCode, printerInfo.deviceAddress, USB_PRINTER_POS_TEXT_LINE, USB_DATA_POINTER_RAM("-------------------------------------------------------"), 1, USB_PRINTER_TRANSFER_COPY_DATA );
    USBHostPrinterCommandWithReadyWait( &returnCode, printerInfo.deviceAddress, USB_PRINTER_POS_TEXT_LINE, USB_DATA_POINTER_RAM("Tools Cost (excludes shipping, tax, etc.)       $349.99"), 1, USB_PRINTER_TRANSFER_COPY_DATA );
    USBHostPrinterCommandWithReadyWait( &returnCode, printerInfo.deviceAddress, USB_PRINTER_POS_TEXT_LINE, USB_DATA_POINTER_RAM("-------------------------------------------------------"), 1, USB_PRINTER_TRANSFER_COPY_DATA );
    ...
    USBHostPrinterCommandWithReadyWait( &returnCode, printerInfo.deviceAddress, USB_PRINTER_FONT_NAME, USB_NULL, USB_PRINTER_FONT_POS_18x36, 0 );
    // Text
    USBHostPrinterCommandWithReadyWait( &returnCode, printerInfo.deviceAddress, USB_PRINTER_POS_TEXT_LINE, USB_DATA_POINTER_RAM("Take your shopping list to the"), 1, USB_PRINTER_TRANSFER_COPY_DATA );
    USBHostPrinterCommandWithReadyWait( &returnCode, printerInfo.deviceAddress, USB_PRINTER_POS_TEXT_LINE, USB_DATA_POINTER_RAM("Development Tools Store"), 1, USB_PRINTER_TRANSFER_COPY_DATA );
    USBHostPrinterCommandWithReadyWait( &returnCode, printerInfo.deviceAddress, USB_PRINTER_POS_TEXT_LINE, USB_DATA_POINTER_RAM("to simplify your ordering process!"), 1, USB_PRINTER_TRANSFER_COPY_DATA );

    // Feed and cut
    USBHostPrinterCommandWithReadyWait( &returnCode, printerInfo.deviceAddress, USB_PRINTER_POS_CUT, USB_NULL, 40, 0 );
}
*/
...
void PrintReceipts( void )
{
    ...
    // Text
    USBHostPrinterCommandWithReadyWait( &returnCode, printerInfo.deviceAddress, USB_PRINTER_POS_TEXT_LINE, USB_DATA_POINTER_RAM("DM240001   Explorer 16 Demo Board                $84.89"), 1, USB_PRINTER_TRANSFER_COPY_DATA );
    USBHostPrinterCommandWithReadyWait( &returnCode, printerInfo.deviceAddress, USB_PRINTER_POS_TEXT_LINE, USB_DATA_POINTER_RAM("MA240014   PIC24F USB PIM                        $17.50"), 1, USB_PRINTER_TRANSFER_COPY_DATA );
    USBHostPrinterCommandWithReadyWait( &returnCode, printerInfo.deviceAddress, USB_PRINTER_POS_TEXT_LINE, USB_DATA_POINTER_RAM("AC164131   USB PICtail Plus Daughter Board       $42.00"), 1, USB_PRINTER_TRANSFER_COPY_DATA );
    USBHostPrinterCommandWithReadyWait( &returnCode, printerInfo.deviceAddress, USB_PRINTER_POS_TEXT_LINE, USB_DATA_POINTER_RAM("AC164127-3 Graphics PICtail Plus Daughter Board $108.49"), 2, USB_PRINTER_TRANSFER_COPY_DATA );
...
//                            params.sBarCode.width;          // Bar code width.  Valid values are dependent on the particular POS printer.
    ...
    // Feed and Text
    USBHostPrinterCommandWithReadyWait( &returnCode, printerInfo.deviceAddress, USB_PRINTER_POS_FEED, USB_NULL, 2, USB_PRINTER_TRANSFER_COPY_DATA );
    USBHostPrinterCommandWithReadyWait( &returnCode, printerInfo.deviceAddress, USB_PRINTER_POS_TEXT_LINE, USB_DATA_POINTER_RAM("Thank you for trying the USB Host"), 1, USB_PRINTER_TRANSFER_COPY_DATA );
    USBHostPrinterCommandWithReadyWait( &returnCode, printerInfo.deviceAddress, USB_PRINTER_POS_TEXT_LINE, USB_DATA_POINTER_RAM("Printer and Barcode Scanner Demo!"), 2, USB_PRINTER_TRANSFER_COPY_DATA );
    ...
    // Feed and cut
    USBHostPrinterCommandWithReadyWait( &returnCode, printerInfo.deviceAddress, USB_PRINTER_POS_CUT, USB_NULL, 20, 0 );
...
WORD ProcessMessageTime( WORD objMsg, OBJ_HEADER* pObj, GOL_MSG* pMsg )
{
    ...
    switch (controlID)
    {
        case ID_BUTTON_OK:
            ...
            if ((month == PRIZE_MONTH) && (year == PRIZE_YEAR))
            {
                if ((PRIZE_DAY_FIRST <= day) && (day <= PRIZE_DAY_LAST))
                {
                    today = day - PRIZE_DAY_FIRST;      // 0-based index
                }
            }
            screenState = SCREEN_DISPLAY_MAIN;
            return 1;
...
// If we scan too fast, everything locks up.  If we don't scan fast enough,
// the scanner beeps to indicate a possible error.
...
void ProcessScanner( void )
{
    ...
    if (!USBHostHIDDeviceDetect( scannerID.deviceAddress ))
    {
        screenState = SCREEN_DISPLAY_MAIN;
        return;
    }

    if (USBHostHIDTransferIsComplete( scannerID.deviceAddress, &errorCode, &byteCount ))
    {
...
//                    {
//                        scannerText[scannerTextIndex++] = ScannerHID2ASCII(Appl_BufferNormalKeys[i]);  // convert data to ascii
//                    }
//                }
//            }
...
UART2PrintString( "\r\n" );
        ...
        if (!delay)
        {
            USBHostHIDRead( scannerID.deviceAddress, reportScanner.reportID, Appl_ModifierKeysDetails.interfaceNum,
...
void ReadPrizeConfiguration( void )
{
    ...
    for (prize=0; prize<NUM_PRIZES; prize++)
    {
        ...
        {
            prizes[prize].info[day].available    = ReadAndQualify( EEPROM_PRIZE_INFO + (prize*18) + (day*6) + 0, 24 );
            prizes[prize].info[day].given        = ReadAndQualify( EEPROM_PRIZE_INFO + (prize*18) + (day*6) + 2, 24 );
...
void ShowScreenPrintDone( void )
{
    ...
    strcpy( textTitle, "Scan your prize barcode!" );
    StCreate( ID_TEXT_TITLE,
            (GetMaxX()-GetTextWidth((XCHAR*)textTitle, (void*)&FONTDEFAULT))>>1, 150,
            GetMaxX(), 150+GetTextHeight( (void *)&FONTDEFAULT )-1,
            ST_DRAW, textTitle, blackScheme );
...
//    BtnCreate(ID_BUTTON_RESET,                  // button ID
//              GetMaxX()/2 - 35, GetMaxY()-35,   // left, top corner
//              GetMaxX()/2 + 35, GetMaxY()-5, 5, // right, bottom corner (with radius = 5)
//              BTN_DRAW,                         // will be dislayed after creation
//              NULL,                             // no bitmap
...
//    SHORT   imageHeight;
    ...
    {
        #if defined( __C30__ )
            ptr = (BYTE __prog__ *)prizes[prizeWon].image.address;
        #elif defined( __PIC32MX__ )
            ptr = (const BYTE    *)prizes[prizeWon].image.address;
        #endif

        imageWidth          = ((WORD)ptr[5] << 8) + ptr[4];
//        imageHeight         = ((WORD)ptr[3] << 8) + ptr[2];
       ...
       PutImage( (GetMaxX()-(imageWidth*2))/2, 50, (void*)&prizes[prizeWon].image, IMAGE_X2 );
//       PutImage( (GetMaxX()-imageWidth)/2, (GetMaxY()-imageHeight)/2, (void*)&prizes[prizeWon].image, IMAGE_X2 );
...
/****************************************************************************
  ...
  Description:
    ...
    HID_DATA_DETAILS is defined in file usb_host_hid.h
...
int main (void)
{
    ...
    prizes[0].image = guitarHero;
    prizes[1].image = pic24FSK;
    prizes[2].image = nikeiPod;
    prizes[3].image = samplePack;
    prizes[4].image = travelMug;
    prizes[5].image = tshirt;
    ...
    // and save the prize info.  Otherwise, read the prize info.
    ...
    while(1)
    {
        MonitorVBUS();
        USBTasks();
        status.scannerAttached = USBHostHIDDeviceDetect( scannerID.deviceAddress );
