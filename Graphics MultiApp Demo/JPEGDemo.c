...
WORD CreateJPEGDemo(void)
{
	...
	pListBox = LbCreate(ID_JPGLISTBOX,    			
              			LBJPGXPOS,LBJPGYPOS,
	...
	pSlider = SldCreate(ID_SLD4LB,
						SLD4LBXPOS, SLD4LBYPOS,
						SLD4LBXPOS+SLD4LBWIDTH,
						SLD4LBYPOS+SLD4LBHEIGHT,
                       	SLD_DRAW|SLD_VERTICAL|SLD_SCROLLBAR,
						2,1,1,				// these are temporary fill items will set to proper values
                       	NULL); 
	if (pSlider == NULL)
		return 0;

    pBtnUp = BtnCreate(	ID_BTNUP4LB,                   
                		BTNUP4LBXPOS, BTNUP4LBYPOS,
    ...
    pBtnDn = BtnCreate(	ID_BTNDN4LB,                   
                		BTNDN4LBXPOS, BTNDN4LBYPOS,
        ...
        case ID_JPGLISTBOX:
        	if (pMsg->uiEvent == EVENT_MOVE) {
				...
				if(aFolderElement[pItemSel->data].blFolder == 1) {
                	...
                	if(pImgFile != NULL) {
                    	blImageOnScreen = 1;
	                    if(ImageDecode(pImgFile, aFolderElement[pItemSel->data].ImgType, 0, 0, IMG_SCREEN_WIDTH, IMG_SCREEN_HEIGHT, 0, NULL, NULL)) {
                        	DisplayErrorInfo();
                    	}
                    	IMG_FCLOSE(pImgFile);
                	}
...
BYTE ShowNextSlide(void)
{
    ...
    if(pImgFile != NULL)
    {
        if(ImageDecode(pImgFile, aFolderElement[NextSlide].ImgType, 0, 0, IMG_SCREEN_WIDTH, IMG_SCREEN_HEIGHT, 0, NULL, NULL)) 
        {
            DisplayErrorInfo();
        }    
        IMG_FCLOSE(pImgFile);
