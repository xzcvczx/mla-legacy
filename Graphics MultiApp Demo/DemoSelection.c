...
static char prevPage = 0;
    ...
    switch(page){
        case 0:
            CreateDemoSelectionPage0();
            break;
        case 1:
            CreateDemoSelectionPage1();
            break;
        default:
            page = 0;
            CreateDemoSelectionPage0();
...
#define ICONBTNINITSTATE  (BTN_DRAW|BTN_TEXTBOTTOM)
...
void CreateDemoSelectionPage0(){
    ...
    BtnCreate(ID_BUTTON1, 										// button ID 
              ...
              iconScheme);              						// use alternate scheme
...
void CheckDemoStatus(void) {
	...
	if (page == 1) {					    					// check if we are in the correct page
    	...
    	if (usbMSDStatus == TRUE)  {							// check if the media is attached
	    	if (GetState(pBtn, BTN_DISABLED) == BTN_DISABLED) {	// enable the button/icon so user can select 
    	...
    	} else if (usbMSDStatus == FALSE) {						// check if media is not attached
	    	if (GetState(pBtn, BTN_DISABLED) != BTN_DISABLED) {	// disable the button/icon, user cannot select
	...
	if (page == 1) {					    					// check if we are in the correct page
    	...
    	if (sdCardStatus == TRUE)  {							// check if the media is attached
	    	if (GetState(pBtn, BTN_DISABLED) == BTN_DISABLED) {	// enable the button/icon so user can select 
				BtnSetBitmap(pBtn, (void*)&ICON11); 
    			ClrState(pBtn, BTN_DISABLED);
			    SetState(pBtn, BTN_DRAW);						// redraw the button to update the state and icon used
    		}
    	} else if (sdCardStatus == FALSE) {						// check if media is not attached
	    	if (GetState(pBtn, BTN_DISABLED) != BTN_DISABLED) {	// disable the button/icon, user cannot select
